...
사용자의 조작으로 값이 바뀔 때 저장하는 통로는 `saveState()` 하나다. `writeStorage()`로
`localStorage`에 쓰고, 실패하면 `notifySaveFailed()`가 안내를 띄우되 **예외를 밖으로 던지지
않는다** — 던지면 같은 처리 안의 뒤따르는 화면 갱신이 중단된다. 쓰기가 성공하면 저장 시각을
남기고 클라우드 전송을 예약한다.

가계부 내용을 `localStorage`에 쓰는 자리는 그 밖에 셋 더 있고, 모두 **들어온 데이터를 받아
적는** 자리다: 기기 저장분을 읽어 정규화한 결과를 되쓰는 자리, 클라우드에서 받은 내용을 적는
자리, 백업 파일을 가져와 통째로 바꾸는 자리. 이 셋은 `saveState()`를 지나지 않는다 — 방금 받은
내용을 그대로 적는 것이라 클라우드로 되돌려 보낼 이유가 없기 때문이다.
